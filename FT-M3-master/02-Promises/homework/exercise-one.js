...
function problemC () {
  ...
  promisifiedReadFile('poem-one/stanza-02.txt')
  .then (st2 => { //capturo valor de la promesa 
    blue(st2);
    return promisifiedReadFile('poem-one/stanza-03.txt'); // me retorna una promesa 
  })
  .then(st3 => { //captura el response de la promesa anterior(return)
    blue(st3);
    console.log('done');
  });
...
function problemE () {
  ...
  // ???
  promisifiedReadFile('poem-one/stanza-03.txt')
    .then(st3 => { //capturo valor de la promesa 
      blue(st3);
      return promisifiedReadFile('poem-one/wrong-file-name.txt'); // me retorna una promesa de leer este <PENDING>
    })
    .then(st4 => { //captura el response de la promesa anterior(return)
      blue(st4);
    }, err => {
      magenta(new Error(err))
    });
...
function problemF () {
  ...
  promisifiedReadFile('poem-one/stanza-03.txt')
    .then(st3 => { //capturo valor de la promesa 
      blue(st3);
      return promisifiedReadFile('poem-one/stanza-04.txt'); // me retorna una promesa de leer este <PENDING>
    })
    .then(st4 => { //captura el response de la promesa anterior(return)
      blue(st4);
      // console.log('done');
    ...
    .finally(() => { // cualquiera de los dos caminos que tomó, hacé algo
      console.log('done');
    });
}
